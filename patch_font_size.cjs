...
// Be careful with global text-xs since it might match sm:text-xs or text-xs sm:text-sm
content = content.replace(/text-xs/g, 'text-sm');
// fix any unintended `sm:text-sm` where it was `sm:text-xs` if that happened
content = content.replace(/sm:text-sm sm:text-sm/g, 'sm:text-sm');
