...
export default function GestionHome({ setView }: { setView: (v: any) => void }) {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <h1 className="text-2xl font-bold text-slate-800 font-lora mb-6 text-center">Panel de Gestión</h1>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {cards.map(c => (
                    <button key={c.id} onClick={() => setView(c.id)}
                        className={`flex flex-col items-center justify-center p-6 rounded-xl border-2 bg-white hover:shadow-md transition-all active:scale-95 group ${c.color}`}>
