const fs = require('fs');
const lines = fs.readFileSync('components/cliente/ClienteApp.tsx', 'utf8').split('\n');

const replacement = `                                        className={\`flex items-center gap-1 transition-all duration-200 rounded-md px-1.5 py-0.5 border \${config.name.toLowerCase().includes('kanala') ? (isSelected ? 'bg-neutral-800 border-white shadow-sm ring-1 ring-neutral-700 z-10 text-white' : 'bg-transparent border-white/10 hover:border-white/30 text-white/60') : (isSelected ? 'bg-white border-slate-800 shadow-sm ring-1 ring-slate-200 z-10' : 'bg-white/40 border-slate-100 hover:border-slate-300')}\`}
                                    >
...
const newLines = [
    ...lines.slice(0, 287), // lines 1 to 287 (index 0 to 286)
    replacement,
    ...lines.slice(300) // lines 301 onwards (index 300 onwards, line 301 is empty line before div class flex justify center)
];

fs.writeFileSync('components/cliente/ClienteApp.tsx', newLines.join('\n'));
