...
// The file currently has literally: className={`\${(mode === 'carta' || mode === 'menu') ? 'w-[35%]' : 'flex-grow'} min-w-0`}
content = content.replace(
    /className=\{\`\\\$\{\(mode === 'carta' \|\| mode === 'menu'\) \? 'w-\[35\%\]' : 'flex-grow'\} min-w-0 flex items-center gap-2 text-left\`\}/g,
    "className={`\\${mode === 'carta' || mode === 'menu' ? 'w-[35%]' : 'flex-grow'} min-w-0 flex items-center gap-2 text-left`}"
);

content = content.replace(
    /className=\{\`\\\$\{\(mode === 'carta' \|\| mode === 'menu'\) \? 'w-\[35\%\]' : 'flex-grow'\} min-w-0\`\}/g,
