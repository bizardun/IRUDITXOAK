...
1. **Respetar Traducciones Oficiales (No Inventar Nada):** Al analizar o traducir platos, la IA **SIEMPRE** debe verificar primero de forma dinámica la página web oficial del restaurante (configurada en `officialWebsite`). Si la web es multilingüe, la IA debe buscar las traducciones exactas en las rutas correspondientes del idioma (ej. `/eu/`, `/en/`, `/fr/`). Si la web oficial ya tiene una traducción para ese plato, la IA **DEBE PRESERVAR Y UTILIZAR ESA TRADUCCIÓN EXACTA** (ej. mostrar literalmente lo que pone, sin alterar una sola letra ni inventar sinónimos).
2. **Uso de Elhuyar para Euskera:** Solo en el caso en el que la web no disponga de traducción al euskera (o sea un plato nuevo), la IA debe generar una traducción usando Google Search para contrastarla con el Diccionario Elhuyar (https://www.euskadi.eus/diccionario-elhuyar/).
